...
import os 
import xlsxwriter # To write the Data

# Set working directory if you want to below:
os.chdir("C:/Users/ArorAb01/Documents/My Work/Learning python/")
...
url1="http://www.imdb.com/search/title?sort=num_votes,desc&start="
url2="&title_type=feature&year=1950,2012"

itera=range(0,int(frec))
imdb=pd.DataFrame()
for x in itera:
    c=50*x+1
    url=url1+str(c)+url2
...
path=input('Please enter path to save the data(forward slash format):') # Fetching the number of records

writer=pd.ExcelWriter(str(path+'IMDBDATA STORY NEW TAGS V2.xlsx'))
imdb.to_excel(writer,'imdb')
writer.save()